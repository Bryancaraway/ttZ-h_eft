...
wc_seight_ffl = { # 68% CL lower, upper with respect to best-fit
    ...
    'fixed':{'cbW' : [-3.24,3.18],'cptb': [-4.98,6.02],'cpt' : [-7.12,2.43],  'ctp' : [6.60,23.62],'ctZ' : [-0.57,0.63],'ctW' : [-0.59,0.50],'cpQ3': [-2.40,1.36],'cpQM': [-3.07,4.93],}, # 
    ...
    'float':{'cbW' : [-3.34,3.39],'cptb': [-4.88,6.95],'cpt' : [-7.53,3.51],  'ctp' : [6.96,23.13],'ctZ' : [-1.02,1.01],'ctW' : [-0.96,0.90],'cpQ3': [-2.43,2.03],'cpQM': [-4.18,5.54],}, # 
}
wc_nfive_ffl = { # 95% CL lower, upper with respect to best-fit
    #'fixed':{'cbW' : [-4.46,4.41],'cptb': [-9.88,10.75],'cpt' : [-12.02,6.32], 'ctp' : [0.25,30.04],'ctZ' : [-1.05,1.11],'ctW' : [-1.05,0.96],'cpQ3': [-4.13,3.04],'cpQM': [-6.55,8.72],},# old toppt
    ...
    #'float':{'cbW' : [-4.54,4.54],'cptb': [-10.19,11.65],'cpt' : [-12.72,7.91],'ctp' : [0.55,29.50],'ctZ' : [-1.68,1.67],'ctW' : [-1.63,1.57],'cpQ3': [-4.44,3.91],'cpQM': [-8.27,9.93],},
...
def beginPlt(year=None):
    ...
    CMSlabel(fig,ax,lumi= round(cfg.Lumi[year],1) if year is not None else round(cfg.Lumi['Total']), opt='', fontsize=14)
...
def endPlt(fig,ax):
    ...
    ax.set_xlabel(r'WC confidence interval \raisebox{0.25ex}{[}$\text{TeV}^\mathsf{-2}$\raisebox{0.25ex}{]}', usetex=True)
    ...
    leg_68 = ax.legend(handles[2:], labels, loc='upper right', bbox_to_anchor=(1.03, 0.40), fontsize=10, framealpha=1, title='68\% CL interval')
