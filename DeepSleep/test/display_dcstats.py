...
sample_list = ['tt_B','TTBar'] + ttZ_list + ttH_list


@save_pdf(f'{ch_name}_mcstats.pdf')
def main():
    for y in cfg.Years:
        mcstat = Plot_mcstats(y)
        mcstat.makeplots()
...
class Plot_mcstats:
    ...
    def load_hists(self):
        with uproot.open(self.roofile) as roo:
            ...
            self.hist_vals['ttZ'] = sum([self.hist_vals[ttZ] for ttZ in  ttZ_list])
            self.hist_vals['ttH'] = sum([self.hist_vals[ttH] for ttH in  ttH_list])
    ...
    def endPlt(self, fig, ax):
        fig.suptitle(f'MC stat in channel {ch_name}, {self.year}')
