...
f_dir = '/cms/data/store/user/bcaraway/NanoAODv7/PreProcessed/2016/TTZToQQ_2016.txt'

def main():
    with open(f_dir,'r') as _f :
        f_list = [l.replace('root://kodiak-se.baylor.edu//','/cms/data/').rstrip('\n') for l in  _f.readlines()]
        c_list = [[f_list[i],f_list[i+1]] for i in range(0 if len(f_list)%2 == 0 else 1,len(f_list),2)]
        pool = Pool()
        out_list = pool.map(__worker, c_list) + ([f_list[0].replace('/cms/data/','root://kodiak-se.baylor.edu//')+'\n'] if len(f_list)%2 == 1 else [])
