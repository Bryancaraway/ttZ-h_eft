...
class AnaVars:
    ...
    def __init__(self, year, isData, jec_sys=None):
        ...
        #
        q_rtc = (lambda k: self.rtc_dict.get(k,k))
        self.var_fact = {
            'Jet_pt'             : f"Jet_pt{self.jec_dict['norm'][self.jec_sys]}",
            'Jet_mass'           : f"Jet_pt{self.jec_dict['norm'][self.jec_sys]}",
            f'Jet_pt{self.LC}'   : f"Jet_pt{self.jec_dict['norm'][self.jec_sys]}{self.LC}",
            f'Jet_mass{self.LC}' : f"Jet_pt{self.jec_dict['norm'][self.jec_sys]}{self.LC}",
            f'FatJet_pt{self.LC}': f"FatJet_pt{self.jec_dict['norm'][self.jec_sys]}{self.LC}",
            f'FatJet_msoftdrop{self.LC}': f"FatJet_msoftdrop{self.jec_dict['norm'][self.jec_sys]}{self.LC}",
        ...
        #
        self.q_fact = (lambda k: self.var_fact.get(k,k))
        #
    ...
    def __getitem__(self,key):
        if key in self.allowed_keys: return getattr(self,key)
            ...
            try:
                return self.var_fact.get(key,key)
            except:
