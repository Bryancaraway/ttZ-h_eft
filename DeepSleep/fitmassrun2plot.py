...
def initDF(data, add_cut=(lambda _df: _df['Zh_pt'] >= 0), add_cut_str='', years=cfg.Years):
    ...
    fig.text(0.235,0.86, f'mean:{popt[4]:>10.2f}$\pm${pcov[4,4]**.5:.2f}\n$\sigma$:{abs(popt[5]):>10.3f}$\pm${pcov[5,5]**.5:.3f}', 
             usetex=True, fontsize=10, color='r')
    #
    ...
    fig.text(0.235,0.78, f'mean:{popt[4]:>10.2f}$\pm${pcov[4,4]**.5:.2f}\n$\sigma$:{abs(popt[5]):>10.3f}$\pm${pcov[5,5]**.5:.3f}', 
             usetex=True, fontsize=10, color='b')
    #print(p0)
    #print(popt)
...
def endplt(fig,ax,add_cut_str,years=None):
    ...
    CMSlabel(fig=fig, ax=ax, opt='Simulation', lumi=round(cfg.Lumi[years[0]],1) if len(years) == 1 else None)
    #fig.text(0.635,0.66, rf'{{{add_cut_str}}} GeV', usetex=True, fontsize=10)
    fig.text(0.685,0.60, f'{add_cut_str} GeV', usetex=True, fontsize=10)
    #fig.text(0.635,0.62, r'DNN score $>0.80$', usetex=True, fontsize=10)
    ax.set_xlabel(rf'${{m}}_{{\mathrm{{SD({{{opt}}})}}}}^{{\mathrm{{AK8\;jet}}}}$ [GeV]', fontsize = 10, usetex=True)
    ...
    ax.legend(handles,labels, framealpha = 0, ncol=2, fontsize=10, loc='upper right')
    plt.tight_layout()
    #plt.show()
