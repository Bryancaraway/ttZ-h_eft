    ...
    for y in years:
        ...
        with uproot.open(roofile) as roo:
            ...
            for k in histos.keys():
                ...
                sys = re.search(r'(_(pref|mu|trigeff|btglf|btghf|lepsf))?_[a-zA-Z0-9]*(Up|Down|On)',process)
                if sys is not None: 
                    sys= sys.group()
...
class PlotSystematics:
    ...
    def __init__(self,process_list,uncert,bins,histos,year):
        ...
        # methods 
        # add pdf-er here
        pdf = matpdf.PdfPages(f"pdf/datacard_{self.year}_sys.pdf")        
